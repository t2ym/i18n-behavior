...
assert.equal(getProperty(nodes[0],p),translate(params.effectiveLang,p,childPath[p]),p+' is set as '+translate(params.effectiveLang,p,childPath[p]));}}//console.log(childPath);
});});}(params.setup?teardown:suiteTeardown)(function(){restoreFixture(params.fixture);});});});};const bundledImportMeta$6={...import.meta,url:new URL('./template-default-lang/template-default-lang-element.js',import.meta.url).href};const $_documentContainer$3=document.createElement('template');$_documentContainer$3.innerHTML=`<template id="template-default-lang-element" lang="fr">
  ...
  </template>`;document.head.appendChild($_documentContainer$3.content);switch(syntax){default:case'mixin':{class TemplateDefaultLangElement extends Mixins.Logger(Mixins.Localizable(Polymer.LegacyElement)){static get importMeta(){return bundledImportMeta$6;}static get template(){return(t=>{t.setAttribute("lang","fr");return t;})((t=>{t.setAttribute("localizable-text","embedded");return t;})(html`{{text.text}}<div><div></div></div><!-- nested empty div -->
    <span id="whitespace"> &nbsp; </span>
...
},attached:function(){},_langUpdated:function(e){if(dom(e).rootTarget===this&&this.text){this.model=deepcopy(this.text.model);}}});}break;}const bundledImportMeta$7={...import.meta,url:new URL('./template-default-lang/null-template-default-lang-element.js',import.meta.url).href};const $_documentContainer$4=document.createElement('template');$_documentContainer$4.innerHTML=`<template id="null-template-default-lang-element" lang="">
