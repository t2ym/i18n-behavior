...
});});}(params.setup?teardown:suiteTeardown)(function(){restoreFixture(params.fixture);});});});};const bundledImportMeta$6={...import.meta,url:new URL('./edge-case/empty-element.js',import.meta.url).href};const $_documentContainer$3=document.createElement('template');$_documentContainer$3.innerHTML=`<template id="empty-element">
  </template>`;document.head.appendChild($_documentContainer$3.content);switch(syntax){default:case'mixin':{class EmptyElement extends Mixins.Localizable(Polymer.LegacyElement){static get importMeta(){return bundledImportMeta$6;}static get template(){return(t=>{t.setAttribute("localizable-text","embedded");return t;})(html`
...
`),is:'complex-compound-binding-element',behaviors:[BehaviorsStore.I18nBehavior],listeners:{'lang-updated':'_langUpdated'},_langUpdated:function(e){console.log('complex-compound-binding-element: lang-updated lang = '+this.lang+' effectiveLang = '+this.effectiveLang);window.setTimeout(function(){Array.prototype.forEach.call(this.root.querySelectorAll('i18n-format'),function(node){if(!node.elements){console.log('elements are missing',node);node.ready();}node.render();});console.log(this.is+' local-dom-ready ');this.fire('local-dom-ready');}.bind(this),1);}});}break;}const bundledImportMeta$9={...import.meta,url:new URL('./edge-case/advanced-binding-element.js',import.meta.url).href};const $_documentContainer$5=document.createElement('template');$_documentContainer$5.innerHTML=`<template id="advanced-binding-element">
  ...
  </template>`;document.head.appendChild($_documentContainer$5.content);switch(syntax){default:case'mixin':{class AdvancedBindingElement extends Mixins.Localizable(Polymer.LegacyElement){static get importMeta(){return bundledImportMeta$9;}static get template(){return(t=>{t.setAttribute("localizable-text","embedded");return t;})(html`
    <span id="status">{{tr(status,text.statusMessages)}}</span>
