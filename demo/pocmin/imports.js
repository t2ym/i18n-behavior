...
if(!content._notes){content._notes=[];// TODO(sorvell): whitespace and processAnnotations need to be factored
// into plugins
// TODO(kschaaf): template should be threaded through rather than implied state
currentTemplate=currentTemplate||template;parseNodeAnnotations(content,content._notes,template.hasAttribute('strip-whitespace'));}return content._notes;}// add annotations gleaned from subtree at `node` to `list`
...
function parseTextNodeAnnotation(node,list){var parts=parseBindings(node.textContent);if(parts){// Initialize the textContent with any literal parts
// NOTE: default to a space here so the textNode remains; some browsers
// (IE) evacipate an empty textNode following cloneNode/importNode.
node.textContent=literalFromParts(parts)||' ';var note={bindings:[{kind:'text',name:'textContent',parts:parts,isCompound:parts.length!==1}]};list.push(note);return note;}}// add annotations gleaned from Element `node` to `list`
...
prepElement(element);}if(note.bindings.length||note.events.length||note.id){list.push(note);}return note;}// add annotations gleaned from children of `root` to `list`, `root`'s
...
var propertyName=Polymer.CaseMap.dashToCamelCase(name);if(kind==='property'){name=propertyName;}return{kind:kind,name:name,propertyName:propertyName,parts:parts,literal:literal,isCompound:parts.length!==1};}}// TODO(sorvell): this should be factored into a plugin
...
/**
       ...
       */},{key:'importHref',value:function importHref(href,onload,onerror,optAsync){var l=document.createElement('link');l.rel='import';l.href=href;optAsync=Boolean(optAsync);if(optAsync){l.setAttribute('async','');}var self=this;if(onload){l.onload=function(e){// NOTE: Push load handler until after `HTMLImports.whenReady`,
...
if(window.HTMLImports){// wait until any other pending imports are ready
HTMLImports.whenReady(function(){// wait until the CustomElements polyfill has upgraded elements.
// (needed because CE's whenReady is installed afer this one)
setTimeout(function(){onload.call(self,e);});});}else{onload.call(self,e);}};}if(onerror){l.onerror=function(e){onerror.call(self,e);};}document.head.appendChild(l);return l;}/**
...
var OPEN_BRACKET='(\\[\\[|{{)'+'\\s*';var CLOSE_BRACKET='(?:]]|}})';var NEGATE='(?:(!)\\s*)?';// Group 2
var EXPRESSION=OPEN_BRACKET+NEGATE+BINDING+CLOSE_BRACKET;return new RegExp(EXPRESSION,"g");}()};if(!Polymer.Element&&Polymer.Annotations._bindingRegex.toString()===_bindingRegex_patch._bindingRegex_1_2_4.toString()){console.log('Fixing Polymer issue#3349 by patching Polymer.Annotations._bindingRegex for Polymer 1.2.4');Polymer.Annotations._bindingRegex=_bindingRegex_patch._bindingRegex_1_2_4_patched;}})();/**
@license
...
*/function MakePromise(asap){function Promise(fn){if(_typeof(this)!=='object'||typeof fn!=='function')throw new TypeError();this._state=null;this._value=null;this._deferreds=[];doResolve(fn,resolve.bind(this),reject.bind(this));}function handle(deferred){var me=this;if(this._state===null){this._deferreds.push(deferred);return;}asap(function(){var cb=me._state?deferred.onFulfilled:deferred.onRejected;if(typeof cb!=='function'){(me._state?deferred.resolve:deferred.reject)(me._value);return;}var ret;try{ret=cb(me._value);}catch(e){deferred.reject(e);return;}deferred.resolve(ret);});}function resolve(newValue){try{//Promise Resolution Procedure: https://github.com/promises-aplus/promises-spec#the-promise-resolution-procedure
if(newValue===this)throw new TypeError();if(newValue&&((typeof newValue==='undefined'?'undefined':_typeof(newValue))==='object'||typeof newValue==='function')){var then=newValue.then;if(typeof then==='function'){doResolve(then.bind(newValue),resolve.bind(this),reject.bind(this));return;}}this._state=true;this._value=newValue;finale.call(this);}catch(e){reject.call(this,e);}}function reject(newValue){this._state=false;this._value=newValue;finale.call(this);}function finale(){for(var i=0,len=this._deferreds.length;i<len;i++){handle.call(this,this._deferreds[i]);}this._deferreds=null;}/**
	 * Take a potentially misbehaving resolver function and make sure
	 * onFulfilled and onRejected are only called once.
	 *
	 * Makes no guarantees about asynchrony.
	 */function doResolve(fn,onFulfilled,onRejected){var done=false;try{fn(function(value){if(done)return;done=true;onFulfilled(value);},function(reason){if(done)return;done=true;onRejected(reason);});}catch(ex){if(done)return;done=true;onRejected(ex);}}Promise.prototype['catch']=function(onRejected){return this.then(null,onRejected);};Promise.prototype.then=function(onFulfilled,onRejected){var me=this;return new Promise(function(resolve,reject){handle.call(me,{onFulfilled:onFulfilled,onRejected:onRejected,resolve:resolve,reject:reject});});};Promise.resolve=function(value){if(value&&(typeof value==='undefined'?'undefined':_typeof(value))==='object'&&value.constructor===Promise){return value;}return new Promise(function(resolve){resolve(value);});};Promise.reject=function(value){return new Promise(function(resolve,reject){reject(value);});};return Promise;}if(typeof module!=='undefined'){module.exports=MakePromise;};if(!window.Promise){window.Promise=MakePromise(Polymer.Base.async);};'use strict';Polymer({is:'iron-request',hostAttributes:{hidden:true},properties:{/**
...
if((Polymer.Element?mutation.target:Polymer.dom(mutation.target)).parentNode.tagName.toLowerCase()!=='i18n-number'||typeof(Polymer.Element?mutation.target:Polymer.dom(mutation.target)).parentNode.formatted!=='undefined'){this.render();}break;case'attributes':if(mutation.attributeName==='lang'){this._lang=this.lang;}break;default:/* istanbul ignore next: mutation.type is always characterData or attributes */break;}},this);},/**
...
if(!this._htmlLangMutationObserver){this._htmlLangMutationObserverCallbackBindThis=this._htmlLangMutationObserverCallback.bind(this);this._htmlLangMutationObserver=new MutationObserver(this._htmlLangMutationObserverCallbackBindThis);}this._htmlLangMutationObserver.observe(html,{attributes:true});},/**
     ...
     */_disconnect:function _disconnect(){if(this._htmlLangMutationObserver){this._htmlLangMutationObserver.disconnect();}}});};if(!window.HTMLImports||HTMLImports.hasNative){registerI18nPreference();}else{HTMLImports.whenReady(registerI18nPreference);}})(document);(function(){// shared data
...
var paramAttribute=Polymer.Element?'slot':'param';var attributesRepository=(!window.HTMLImports||HTMLImports.hasNative?document.currentScript:document._currentScript).ownerDocument.querySelector('i18n-attr-repo');// set up userPreference
...
switch(result[result.length-1]){case'zh-CN':case'zh-CHS':result.push('zh-Hans');break;case'zh-TW':case'zh-SG':case'zh-HK':case'zh-CHT':result.push('zh-Hant');break;default:break;}}parts.pop();}}return result;},/**
    ...
    *//**
     ...
     *
     * @param {Array} mutations Array of MutationRecord (https://developer.mozilla.org/en-US/docs/Web/API/MutationObserver).
     */_handleLangAttributeChange:function _handleLangAttributeChange(mutations){mutations.forEach(function(mutation){switch(mutation.type){case'attributes':if(mutation.attributeName==='lang'){//console.log('_handleLangAttributeChange lang = ' + this.lang + ' oldValue = ' + mutation.oldValue +
//            ' typeof oldValue = ' + typeof mutation.oldValue);
if(!(_typeof(mutation.oldValue)==='object'&&!mutation.oldValue)&&mutation.oldValue!==this.lang){if(this._lang!==mutation.oldValue){//console.log('assigning this._lang = ' + mutation.oldValue + ' from old value');
...
this._lang=this.lang;}}break;default:/* istanbul ignore next: mutation.type is always attributes */break;}},this);},/**
     ...
     */_langChanged:function _langChanged(lang,oldLang){//console.log(this.id + ':_langChanged lang = ' + lang + ' oldLang = ' + oldLang);
...
if(!isStandardPropertyConfigurable){var _properties=Object.create(null);for(var p in BehaviorsStore.I18nBehavior.properties){if(p==='lang'){_properties._lang=BehaviorsStore.I18nBehavior.properties.lang;}else{_properties[p]=BehaviorsStore.I18nBehavior.properties[p];}}BehaviorsStore.I18nBehavior.properties=_properties;BehaviorsStore.I18nBehavior.properties._lang.reflectToAttribute=false;BehaviorsStore.I18nBehavior.properties.text.computed='_getBundle(_lang)';BehaviorsStore.I18nBehavior._updateEffectiveLang=function(event){if(!Polymer.Element&&Polymer.dom(event).rootTarget===this||Polymer.Element&&event.composedPath()[0]===this){//console.log('lang-updated: _updateEffectiveLang: assigning effectiveLang = ' + this._lang);
this.effectiveLang=this._lang;}};BehaviorsStore.I18nBehavior.hostAttributes={'lang':defaultLang};}if(Polymer.Element){// Polymer 2.x
}else{// Polymer 1.x
/**
...
Polymer.Utils=Polymer.Utils||{};Polymer.Utils.MixinBehavior=function(behavior,Base){var utils=Polymer.Utils;var metaProps={attached:true,detached:true,ready:true,created:true,beforeRegister:true,registered:true,attributeChanged:true,// meta objects
...
.replace(/ /g,'-')// lowercase
.toLowerCase();}},{key:'_i18nPreprocess',value:function _i18nPreprocess(template){if(this.is&&template&&!this._templateLocalizable){this._templateLocalizable=this.prototype._constructDefaultBundle(template);}return template;}},{key:'_rawTemplate',get:function get(){var id=this.is;var name=this.name||(typeof this==='function'?this.toString().replace(/^[\S\s]*?function\s*/,"").replace(/[\s\(\/][\S\s]+$/,""):undefined);if(!id&&name&&name!=='Localizable'&&name!='class'&&name!='_class'){id=this.is=this._uncamelCase(name);}// TODO: template fetching should be flexible to detect super.template
var template=Polymer.DomModule.import(id,'template');if(id&&!template){var current=!window.HTMLImports||HTMLImports.useNative?document.currentScript:document._currentScript;template=(current?current.ownerDocument.querySelector('template[id='+id+']'):null)||document.querySelector('template[id='+id+']');if(template){var domModule=document.createElement('dom-module');var assetUrl=new URL(current.baseURI||(window.currentImport?window.currentImport.baseURI:null)||current.ownerDocument.baseURI);domModule.appendChild(template);domModule.setAttribute('assetpath',assetUrl.pathname.indexOf('.vulcanized.')<0?assetUrl.pathname:template.hasAttribute('assetpath')?template.getAttribute('assetpath'):assetUrl.pathname);domModule.register(id);this._template=template;}}return template;}},{key:'template',get:function get(){return this._i18nPreprocess(this._rawTemplate);}},{key:'_templateLocalizable',get:function get(){return this.hasOwnProperty('__templateLocalizable');},set:function set(value){this.__templateLocalizable=value;}}]);return Localizable;}(Polymer.Utils.MixinBehavior(BehaviorsStore.I18nBehavior,base));};Mixins.I18nBehavior=function(info){return Mixins.Localizable(Polymer.Class(info));};// Logger mixin
...
Object.defineProperty(BaseElements,'I18nElement',{get:function get(){return Mixins.I18nBehavior(Polymer.LegacyElement);}});// i18n-dom-bind based on Polymer/polymer#2.0-preview src/templatizer/dom-bind.html
var I18nDomBind=function(_Mixins$Localizable){_inherits(I18nDomBind,_Mixins$Localizable);function I18nDomBind(){_classCallCheck(this,I18nDomBind);return _possibleConstructorReturn(this,(I18nDomBind.__proto__||Object.getPrototypeOf(I18nDomBind)).apply(this,arguments));}_createClass(I18nDomBind,[{key:'connectedCallback',value:function connectedCallback(){this._fetchStatus=deepcopy({// per custom element
fetchingInstance:null,ajax:null,ajaxLang:null,lastLang:null,fallbackLanguageList:null,targetLang:null,lastResponse:{},rawResponses:{}});this.render();}},{key:'disconnectedCallback',value:function disconnectedCallback(){this._removeChildren();}},{key:'_insertChildren',value:function _insertChildren(){if(this._children){for(var i=0;i<this._children.length;i++){this.parentNode.insertBefore(this._children[i],this);}}}},{key:'_removeChildren',value:function _removeChildren(){if(this._children){for(var i=0;i<this._children.length;i++){this.stamped.appendChild(this._children[i]);}}}},{key:'render',value:function render(){if(!this._children){var template=this.querySelector('template');if(!template){throw new Error('i18n-dom-bind requires a <template> child');}this._templateLocalizable=this._constructDefaultBundle(this.querySelector('template'));this._bindTemplate(template);this.root=this._stampTemplate(template);this.stamped=this.root;this._children=[];for(var n=this.root.firstChild;n;n=n.nextSibling){this._children[this._children.length]=n;}this._flushProperties(this);}this._insertChildren();this.dispatchEvent(new CustomEvent('dom-change'));}}],[{key:'is',get:function get(){return'i18n-dom-bind';}}]);return I18nDomBind;}(Mixins.Localizable(Polymer.LegacyElement));customElements.define(I18nDomBind.is,I18nDomBind);var LocalizableElement=function(_Mixins$Logger){_inherits(LocalizableElement,_Mixins$Logger);function LocalizableElement(){_classCallCheck(this,LocalizableElement);return _possibleConstructorReturn(this,(LocalizableElement.__proto__||Object.getPrototypeOf(LocalizableElement)).apply(this,arguments));}_createClass(LocalizableElement,null,[{key:'is',get:function get(){return'localizable-element';}}]);return LocalizableElement;}(Mixins.Logger(Mixins.Localizable(Polymer.LegacyElement)));customElements.define(LocalizableElement.is,LocalizableElement);var I18nSubclassElement=function(_Mixins$Logger2){_inherits(I18nSubclassElement,_Mixins$Logger2);function I18nSubclassElement(){_classCallCheck(this,I18nSubclassElement);return _possibleConstructorReturn(this,(I18nSubclassElement.__proto__||Object.getPrototypeOf(I18nSubclassElement)).apply(this,arguments));}_createClass(I18nSubclassElement,[{key:'_langUpdated',value:function _langUpdated(){this.model=deepcopy(this.text.model);}}],[{key:'is',get:function get(){return'i18n-subclass-element';}},{key:'config',get:function get(){return{listeners:{'lang-updated':'_langUpdated'}};}}]);return I18nSubclassElement;}(Mixins.Logger(BaseElements.I18nElement));customElements.define(I18nSubclassElement.is,I18nSubclassElement);(function(){function UncamelCase(name){return name// insert a hyphen between lower & upper
.replace(/([a-z0-9])([A-Z])/g,'$1 $2')// space before last upper in a sequence followed by lower
.replace(/\b([A-Z]+)([A-Z])([a-z0-9])/,'$1 $2$3')// replace spaces with hyphens
.replace(/ /g,'-')// lowercase
.toLowerCase();}function functionName(func){return typeof func==='function'?func.toString().replace(/^[\S\s]*?function\s*/,"").replace(/[\s\(\/][\S\s]+$/,""):undefined;}if(!window.Define){Object.defineProperty(window,'Define',{get:function get(){return function(id){return this.PolymerElements?this.PolymerElements[id]:undefined;};},set:function set(proto){/*
        Patterns:
          a) template id {}
          b) template id {is}
          ...
          f) class Is {template}
          g) {is,template}
        */var id;var classId;var obj;var name=proto.name||functionName(proto);var current;// currentScript
var template=null;var previous;// previousSibling template
var cousin;// dom5.serialize output support
current=!window.HTMLImports||HTMLImports.useNative?document.currentScript:document._currentScript;previous=current.previousSibling;while(previous&&!previous.tagName){previous=previous.previousSibling;}if(previous&&previous.tagName!=='template'.toUpperCase()){previous=null;}if(!previous){// search for cousin template
if(current.parentNode.tagName==='body'.toUpperCase()){previous=current.parentNode.previousSibling;while(previous&&!previous.tagName){previous=previous.previousSibling;}if(previous&&previous.tagName.toLowerCase()==='head'){for(var i=0;i<previous.childNodes.length;i++){if(previous.childNodes[i].tagName==='template'.toUpperCase()){cousin=previous.childNodes[i];break;}}}}if(cousin){previous=cousin;}else{previous=null;}}if(!proto.is&&(!name||name==='class'||name==='Define')){if(previous){id=previous.id;if(id){// Pattern a)
template=previous;proto.is=id;}}}else{if(proto.is){id=proto.is;}else if(typeof proto==='function'&&name){// ES6 class
id=UncamelCase(name);classId=name;}var rawTemplate=proto._rawTemplate||proto.template;if(!template&&rawTemplate&&!name){// Pattern g)
template=rawTemplate;}if(!rawTemplate&&!template){// Pattern b), c)
template=current.ownerDocument.querySelector('template[id='+id+']')||document.querySelector('template[id='+id+']');}if(!rawTemplate&&!template&&previous&&!previous.id){// Pattern d)
template=previous;template.id=id;}else{// Pattern e)
}}if(!id){throw'Custom element name is not defined';}// register dom-module
if(template){var domModule=document.createElement('dom-module');var assetUrl=new URL(current.baseURI||(window.currentImport?window.currentImport.baseURI:null)||current.ownerDocument.baseURI);domModule.appendChild(template);domModule.setAttribute('assetpath',assetUrl.pathname.indexOf('.vulcanized.')<0?assetUrl.pathname:template.hasAttribute('assetpath')?template.getAttribute('assetpath'):assetUrl.pathname);domModule.register(id);proto._template=template;proto.finalize();}// register Custom Element
this.PolymerElements=this.PolymerElements||{};classId=classId||id.split('-').map(function(word){return word[0].toUpperCase()+word.substr(1);}).join('');if(this.PolymerElements[id]){console.warn('Discarding duplicate regitration of custom element '+id);}else{window[classId]=proto;customElements.define(id,proto);this.PolymerElements[classId]=this.PolymerElements[id];}return this.PolymerElements[id];}});}})();Define=function(_Mixins$Logger3){_inherits(I18nThinElement,_Mixins$Logger3);function I18nThinElement(){_classCallCheck(this,I18nThinElement);return _possibleConstructorReturn(this,(I18nThinElement.__proto__||Object.getPrototypeOf(I18nThinElement)).apply(this,arguments));}return I18nThinElement;}(Mixins.Logger(BaseElements.I18nElement));Polymer(Mixins.I18nBehavior({is:'i18n-legacy-element'}));
//# sourceMappingURL=sourcemaps/imports.js.map
