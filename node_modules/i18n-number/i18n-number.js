...
function _setupIntlPolyfillLocale(locale, callback) {
  if (!window.IntlPolyfill) {
    ...
    if (intlLibraryLoadingStatus !== 'native') {
      var supported = Intl.NumberFormat.supportedLocalesOf(locale, {
        ...
        if (!script) {
          ...
          var intlLocaleLoadErrorBindThis = function (e) {
            if (e.target === script) {
              ...
              var fallbackSupport = Intl.NumberFormat.supportedLocalesOf(locale, {
                localeMatcher: 'lookup'
              });
          ...
          var enSupport = Intl.NumberFormat.supportedLocalesOf(this.DEFAULT_LANG, {
            localeMatcher: 'lookup'
          });
...
Polymer({
  ...
  is: 'i18n-number',
  _template: (t => {
    t.setAttribute('strip-whitespace', '');
    return t;
  ...
  _setupObservers: function () {
    ...
    this.nodeObserver = dom(this).observeNodes(function (info) {
      ...
      do {
        if (info.addedNodes[i] && info.addedNodes[i].nodeType === info.addedNodes[i].TEXT_NODE) {
          this.textNode = info.addedNodes[i];
          this.raw = this.textNode.data; //console.log('i18n-number: text node added with ' + this.raw);
  ...
  _render: function (lang, options, raw, offset) {
    ...
    this.$.number.textContent = this.formatted ? this.formatted : ''; //console.log('i18n-number: _render ' + this.formatted);
